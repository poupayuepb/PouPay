package com.project.poupay.validators;
...
public class FieldValidator {
    ...
    public static boolean validate(TextView field, int type) {
        ...
        if (type == TYPE_USERNAME) {
            ...
            textError = "O nome de usuário deve conter apenas letras maiúsculas e minúsculas, underline e pontos.";
        } else {
            pattern = Pattern.compile("^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#&()–{}:;.',?/*~$^+=<>]).{8,20}$");
            textError = "A senha deve conter pelo menos um carácter minúsculo, um maiúsculo, um número e um carácter especial.";
        }
